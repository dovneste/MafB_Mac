...
```{r}
MafB_target_sign <- read_excel("MafB_target_lemur.xlsx")

MafB_target_sign <- MafB_target_sign$ortholog_name

MafB_target_sign <- intersect(MafB_target_sign, rownames(TBM))

TBM <- AddModuleScore(TBM, features = list(c(MafB_target_sign)), name = "MafB_target_sign")
```
