...
```{r}
MafB_target_danio <- read_excel("MafB_target_danio.xlsx")

MafB_target_danio <- MafB_target_danio$ortholog_name

MafB_target_danio <- intersect(MafB_target_danio, rownames(ZCL))


ZCL <- AddModuleScore(ZCL, features = list(c(MafB_target_danio)), name = "MafB_target_sign")
```
