...
write.table(append(MafB_target_genes, "#MafB_target_genes", 0), file = "MafB_target_genes.grp", row.names = FALSE, col.names = FALSE, quote = FALSE)

MafB_target_genes <- intersect(MafB_target_genes, rownames(sc))

sc <- AddModuleScore(sc, features = list(MafB_target_genes), name = 'Mafb_sign')
...
wilcox.test(SPM_WT$Mafb_sign1, SPM_KO$Mafb_sign1)
```
...
wilcox.test(LPM_WT$Mafb_sign1, LPM_KO$Mafb_sign1)
```
...
wilcox.test(KC_WT$Mafb_sign1, KC_KO$Mafb_sign1)
```
...
wilcox.test(IM_WT$Mafb_sign1, IM_KO$Mafb_sign1)
```
...
wilcox.test(Ly6C_WT$Mafb_sign1, Ly6C_KO$Mafb_sign1)
```
...
wilcox.test(MHC2_WT$Mafb_sign1, MHC2_KO$Mafb_sign1)
```
...
wilcox.test(MG_WT$Mafb_sign1, MG_KO$Mafb_sign1)
```
